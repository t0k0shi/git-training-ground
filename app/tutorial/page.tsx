...
export default function TutorialPage() {
  return (
    <main className="min-h-screen bg-bg">
      {/* ヒーローエリア */}
      <section className="pt-28 pb-12 bg-bg">
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        {/* 前提条件 */}
        <section className="mb-12 bg-paper rounded-xl border border-line shadow-card p-6">
          <h2 className="text-xl font-bold text-ink mb-4">必要なもの</h2>
          <ul className="space-y-3 text-ink-2">
            <li className="flex items-start gap-3">
              <span className="text-accent font-bold mt-0.5">✓</span>
              <span>
                <strong className="text-ink">GitHub アカウント</strong>（
                <a href="https://github.com" className="text-accent underline" target="_blank" rel="noopener noreferrer">github.com</a>{' '}
                で無料作成できます）
              </span>
            </li>
            <li className="flex items-start gap-3">
              <span className="text-accent font-bold mt-0.5">✓</span>
              <span>
                <strong className="text-ink">ブラウザ</strong>（これを読んでいるなら準備OK）
              </span>
            </li>
          </ul>
        ...
        <div className="space-y-0">
          ...
          <StepGuide step={1} title="リポジトリをForkする">
            <p className="mb-3">
              まず、<a href="https://github.com/t0k0shi/git-training-ground" className="text-accent underline" target="_blank" rel="noopener noreferrer">Git Training Ground のリポジトリ</a>を開きます。
            </p>
            <p className="mb-3">
            ...
            <div className="bg-bg-2 rounded-lg border border-line p-4 text-sm">
              <p className="text-ink-2">
                <strong className="text-ink">Forkとは？</strong><br />
          ...
          <StepGuide step={2} title="contributors.json を開く">
            <p className="mb-3">
              Forkしたリポジトリ（自分のアカウントの git-training-ground）で、<code className="bg-ink/5 px-1.5 py-0.5 rounded text-sm">data/contributors.json</code> を開きます。
            </p>
            <p className="mb-3">
          ...
          <StepGuide step={4} title="自分のエントリを追加する">
            ...
            <p className="mb-2 text-sm text-ink font-medium">
              ① 以下のコードを<strong>そのまま</strong>コピーしてください（先頭の <code className="bg-ink/5 px-1 rounded">,</code>（カンマ）も必要です）:
            </p>
            <div className="bg-ink rounded-lg p-4 text-sm font-mono text-white overflow-x-auto mb-4">
              <pre>{`  ,
  {
    "name": "あなたの名前",
    ...
    "joinedAt": "2026-04-24"
            ...
            <p className="mb-2 text-sm text-ink font-medium">
              ② 貼り付ける位置: <strong>配列の最後のエントリの <code className="bg-ink/5 px-1 rounded">{'}'}</code> の直後、<code className="bg-ink/5 px-1 rounded">{']'}</code> の直前</strong>。
            </p>
  ...
  },
  {
    ...
    "joinedAt": "2026-04-24"
            ...
            <div className="bg-bg-2 border border-line rounded-lg p-4 text-sm mb-4">
              <p className="text-ink-2 font-medium mb-2">各フィールドの意味</p>
              <div className="space-y-1 text-ink-2">
                <p><code className="bg-paper border border-line px-1.5 rounded">name</code> — 表示名（ハンドルネーム可）</p>
                <p><code className="bg-paper border border-line px-1.5 rounded">github</code> — GitHub ハンドルまたは URL</p>
                <p><code className="bg-paper border border-line px-1.5 rounded">favoriteColor</code> — カードボーダー色（<code className="bg-paper border border-line px-1 rounded">#RRGGBB</code> 形式）</p>
                <p><code className="bg-paper border border-line px-1.5 rounded">favoriteEmoji</code> — 1 文字の絵文字</p>
                <p><code className="bg-paper border border-line px-1.5 rounded">message</code> — 1 行自己紹介</p>
                <p><code className="bg-paper border border-line px-1.5 rounded">joinedAt</code> — PR を出す日付（<code className="bg-paper border border-line px-1 rounded">YYYY-MM-DD</code>）</p>
              </div>
            </div>

            <div className="bg-bg-2 border border-line rounded-lg p-4 text-sm mb-4">
              <p className="text-ink-2 font-medium mb-2">参考になるサイト</p>
            ...
            <div className="bg-paper border-l-4 border-accent rounded-lg p-4 text-sm">
              <p className="text-ink-2">
                <strong>⚠️ 注意:</strong> 他の人のエントリは消さないでください。
                コピペしたコードの先頭の <code className="bg-ink/5 px-1 rounded">,</code> は必ず残してください（JSON の構文ルール）。
              </p>
            </div>
          </StepGuide>
          ...
          <StepGuide step={5} title="変更をコミットする">
            ...
            <div className="bg-bg-2 rounded-lg border border-line p-4 text-sm space-y-2 mb-3">
              <p><strong className="text-ink">Commit message:</strong> <code className="bg-white px-1.5 py-0.5 rounded">Add &lt;yourname&gt; to contributors</code>（<code className="bg-white px-1 rounded">&lt;yourname&gt;</code> は自分のハンドル名に置換）</p>
              <p><strong className="text-ink">選択:</strong> 「Create a new branch for this commit and start a pull request」を選ぶ</p>
              <p><strong className="text-ink">ブランチ名:</strong> <code className="bg-white px-1.5 py-0.5 rounded">add-&lt;yourname&gt;</code> のように設定（例: <code className="bg-white px-1 rounded">add-oginochihiro</code>）</p>
            </div>
            ...
            <div className="bg-paper border-l-4 border-accent rounded-lg p-4 text-sm mb-3">
              <p className="text-ink-2">
            ...
            <div className="mt-4 bg-paper border-l-4 border-accent rounded-lg p-4 text-sm">
              <p className="text-ink-2">
          ...
          <StepGuide step={6} title="元のリポジトリからPull Requestを作成する">
            <p className="mb-3">
              コミット後、<a href="https://github.com/t0k0shi/git-training-ground" className="text-accent underline" target="_blank" rel="noopener noreferrer">元のリポジトリ（t0k0shi/git-training-ground）</a>を開いてください。
            </p>
            <p className="mb-3">
            ...
            <div className="bg-paper border-l-4 border-accent rounded-lg p-4 text-sm mb-3">
              <p className="text-ink-2">
                <strong>⚠️ 重要:</strong> 必ず元のリポジトリ（<code className="bg-ink/5 px-1 rounded">t0k0shi/git-training-ground</code>）側から操作してください。自分のFork内で操作すると、PRの送り先が自分のForkになってしまいます。
              </p>
            </div>
            <div className="bg-bg-2 rounded-lg border border-line p-4 text-sm">
              <p className="text-ink-2">
                <strong className="text-ink">バナーが表示されない場合は？</strong><br />
                時間が経つとバナーが消えることがあります。その場合は自分のForkページで「<strong>Contribute</strong>」→「<strong>Open pull request</strong>」をクリックするか、「<strong>Pull requests</strong>」タブ →「<strong>New pull request</strong>」→「<strong>compare across forks</strong>」から、base を <code className="bg-ink/5 px-1 rounded">t0k0shi/git-training-ground</code>、compare を自分のフォークのブランチに設定してください。
              </p>
            </div>
          ...
          <StepGuide step={7} title="PRの内容を確認して送信する">
            <p className="mb-3">
              「<strong>Open a pull request</strong>」画面が表示されます。base が <code className="bg-ink/5 px-1.5 py-0.5 rounded text-sm">t0k0shi/git-training-ground</code> になっていることを確認してください。
            </p>
            ...
            <div className="bg-bg-2 rounded-lg border border-line p-4 text-sm space-y-2 mb-3">
              <p><strong className="text-ink">Title:</strong> <code className="bg-white px-1.5 py-0.5 rounded">Add &lt;yourname&gt; to contributors</code>（コミットメッセージが自動入力されます）</p>
              <p><strong className="text-ink">Description:</strong> 空欄でもOKです</p>
            </div>
            <p>
        ...
        {/* コンフリクト誘導リンク */}
        <div className="mt-8 bg-bg-2 border border-line rounded-xl p-6 text-center">
          <p className="text-ink-2 text-sm">
            PRを出した後に「コンフリクト」と表示されたら？
          </p>
          <a
            href="#conflict-resolution"
            className="inline-block mt-2 text-accent font-medium underline text-sm"
          >
            コンフリクトについて →
          </a>
        </div>
        ...
        <section id="conflict-resolution" className="mt-16 scroll-mt-24">
          ...
          {/* 安心メッセージ */}
          <div className="bg-paper border border-line shadow-card rounded-xl p-6 mb-8">
            <p className="text-ink-2 leading-relaxed">
              PRを出した後に「コンフリクトがあります」と表示されることがあります。
              これは、あなたが作業している間に他の人の変更がマージされたためです。
            </p>
            <p className="text-ink-2 font-bold mt-3">
              心配しないでください！
            </p>
          ...
          {/* Step A */}
          <div className="mb-6 bg-paper rounded-xl border border-line p-6">
            <h3 className="text-lg font-bold text-ink mb-3">
              <span className="text-accent mr-2">Step A</span>
              upstream を登録する
            </h3>
          ...
          <div className="mb-6 bg-paper rounded-xl border border-line p-6">
            <h3 className="text-lg font-bold text-ink mb-3">
              <span className="text-accent mr-2">Step B</span>
              upstream の最新を取得する
            </h3>
            <p className="text-ink-2 mb-3">
          ...
          <div className="mb-6 bg-paper rounded-xl border border-line p-6">
            <h3 className="text-lg font-bold text-ink mb-3">
              <span className="text-accent mr-2">Step C</span>
              コンフリクトを解決する
            </h3>
            <p className="text-ink-2 mb-3">
              コンフリクトマーカー（<code className="bg-ink/5 px-1 rounded">{'<<<<<<<'}</code>, <code className="bg-ink/5 px-1 rounded">=======</code>, <code className="bg-ink/5 px-1 rounded">{'>>>>>>>'}</code>）を削除して、自分のエントリと他の人のエントリの両方を残します。JSON のカンマの位置にも注意してください。
            </p>
            <div className="bg-bg-2 rounded-lg border border-line p-4 text-sm mb-3">
              <p className="text-ink-2 font-medium mb-2">変更前（コンフリクトマーカーあり）:</p>
              <pre className="bg-ink text-bg p-3 rounded text-xs leading-relaxed overflow-x-auto">{`[
  {
    "name": "ketts",
    "github": "t0k0shi",
    ...
...
>>>>>>> upstream/main
            ...
            <div className="bg-bg-2 rounded-lg border border-line p-4 text-sm">
              <p className="text-ink-2 font-medium mb-2">変更後（両方のエントリを残す）:</p>
              <pre className="bg-ink text-bg p-3 rounded text-xs leading-relaxed overflow-x-auto">{`[
  {
    "name": "ketts",
    "github": "t0k0shi",
  ...
  },
  {
    ...
    ...
          ...
          <div className="mb-6 bg-paper rounded-xl border border-line p-6">
            <h3 className="text-lg font-bold text-ink mb-3">
              <span className="text-accent mr-2">Step D</span>
              解決をコミットして push する
            </h3>
            <p className="text-ink-2 mb-3">
        ...
        {/* 完了セクション */}
        <section className="mt-16 text-center bg-paper border border-line shadow-card-lg rounded-2xl p-10">
          <h2 className="text-2xl font-bold text-ink mb-3">PRがマージされたら...</h2>
